...
NK_INTERN void
nk_emit_hotspot(struct nk_context* ctx, struct nk_rect rect, enum nk_style_cursor cursor, int level)
{
    if (ctx->hotspotFunction && ctx->hotspotHandle.ptr)
        ctx->hotspotFunction(ctx->hotspotHandle, { rect, cursor, level });
}
    ...
    if (edit->active)
    {
        ...
        }
        {
            ...
            if (cursor_follow)
            {
                ...
                if (!(flags & NK_EDIT_NO_HORIZONTAL_SCROLL)) {
                    ...
                    while (cursor_pos.x < edit->scrollbar.x)
                        edit->scrollbar.x -= scroll_increment;// (float)(int)NK_MAX(0.0f, cursor_pos.x - scroll_increment);
                    //if (cursor_pos.x >= edit->scrollbar.x + area.w)
                    //    edit->scrollbar.x = (float)(int)NK_MAX(0.0f, cursor_pos.x);
                    // JSandusky: stay ahead so we don't act janky jumping to a "empty looking" view
                    while (cursor_pos.x >= edit->scrollbar.x + area.w - scroll_increment)
                        edit->scrollbar.x += scroll_increment;// (float)(int)NK_MAX(0.0f, cursor_pos.x - scroll_increment);
                    if (edit->scrollbar.x < 0)
                        edit->scrollbar.x = 0;
                }
                ...
                if (flags & NK_EDIT_MULTILINE) {
                    ...
                    // JSandusky: stay ahead so we don't jump to anything janky
        ...
        if (edit->select_start == edit->select_end)
        {
            if (edit->cursor >= nk_str_len(&edit->string) ||
                ...
                // JSandusky: If we're in insert mode then don't tint the text oddly
                if (edit->mode == NK_TEXT_EDIT_MODE_INSERT)
                    txt.text = style->text_normal;
                else
                    txt.text = cursor_text_color;

                // JSandusky: when in insert mode, draw the cursor as a narrow line
                if (edit->mode == NK_TEXT_EDIT_MODE_INSERT)
                    label.w = 2;
    ...
    const char *name, int len, const struct nk_user_font *font)
{
    ...
    // JSandusky: only take up a colon for labels, clip the label
    for (int i = 0; i < len; ++i)
    {
        if (name[i] == ':')
        {
    ...
    nk_widget_text(out, *label, name, len, &text, /*NK_TEXT_CENTERED*/NK_TEXT_LEFT, font);
}
...
nk_do_property(struct nk_context* ctx, nk_flags *ws,
    ...
    enum nk_button_behavior behavior, nk_prop_color_flags flags, nk_color color, int forceActive)
{
    ...
    struct nk_rect dragArea = property;
    ...
    /* edit */
    if ((*state == NK_PROPERTY_EDIT) || forceActive) {
    ...
    // JSandusky
    dragArea.w = edit.x - dragArea.x;
    nk_emit_hotspot(ctx, dragArea, NK_CURSOR_RESIZE_HORIZONTAL, 0);
    ...
    {
    case NK_PROPERTY_INT: {
        factor = ((float)variant->value.i - (float)variant->min_value.i) / ((float)variant->max_value.i - (float)variant->min_value.i);
    } break;
    case NK_PROPERTY_FLOAT: {
        factor = (variant->value.f - variant->min_value.f) / (variant->max_value.f - variant->min_value.f);
    }  break;
    case NK_PROPERTY_DOUBLE: {
        factor = (variant->value.d - variant->min_value.d) / (variant->max_value.d - variant->min_value.d);
    }break;
    }
    if (flags == NK_PROP_COLOR_PERCENT)
        nk_stroke_line(out, property.x + 5, property.y + property.h, property.x + 5 + (property.w - 10), property.y + property.h, 2, color);
    else if (flags == NK_PROP_COLOR_FULL)
        nk_stroke_line(out, property.x + 5, property.y + property.h, property.x + 5 + (property.w - 10), property.y + property.h, 2, color);
    ...
    nk_emit_hotspot(ctx, edit, NK_CURSOR_TEXT, 0);
...
nk_style_from_table(struct nk_context *ctx, const struct nk_color *table)
{
    ...
    edit->cursor_text_normal = table[NK_COLOR_TEXT];//table[NK_COLOR_EDIT];
    edit->cursor_text_hover = table[NK_COLOR_TEXT];// table[NK_COLOR_EDIT];
...
nk_free(struct nk_context *ctx)
{
    ...
    ctx->build = 0;
    ctx->begin = 0;
    ctx->cursor_style = NK_CURSOR_ARROW;
    ctx->end = 0;
    ctx->active = 0;
    ctx->current = 0;
    ctx->freelist = 0;
...
nk_panel_begin(struct nk_context *ctx, const char *title, enum nk_panel_type panel_type)
{
    ...
    if ((win->flags & NK_WINDOW_MOVABLE) && !(win->flags & NK_WINDOW_ROM)) {
        ...
        if (left_mouse_down && left_mouse_click_in_cursor) {
            ...
            ctx->cursor_style = NK_CURSOR_MOVE;
        }
    }
    ...
    if ((layout->flags & NK_WINDOW_SCALABLE) && in && !(layout->flags & NK_WINDOW_MINIMIZED))
    {
        ...
        if (!(window->flags & NK_WINDOW_ROM)) {
            ...
            if (left_mouse_down && left_mouse_click_in_scaler) {
                ...
                ctx->cursor_style = NK_CURSOR_RESIZE_TOP_RIGHT_DOWN_LEFT;
...
nk_layout_row_template_push_dynamic(struct nk_context *ctx)
{
    ...
    layout->row.templates[layout->row.columns++] = { -1.0f, NK_LAYOUT_TOP, NK_LAYOUT_LEFT };
}
...
nk_layout_row_template_push_variable(struct nk_context *ctx, float min_width)
{
    ...
    if (layout->row.type != NK_LAYOUT_TEMPLATE) return;
    if (layout->row.columns >= NK_MAX_LAYOUT_ROW_TEMPLATE_COLUMNS) return;
    layout->row.templates[layout->row.columns++] = { -min_width, NK_LAYOUT_TOP, NK_LAYOUT_LEFT };
}
...
nk_layout_row_template_push_static(struct nk_context *ctx, float width)
{
    ...
    if (layout->row.type != NK_LAYOUT_TEMPLATE) return;
    if (layout->row.columns >= NK_MAX_LAYOUT_ROW_TEMPLATE_COLUMNS) return;
    layout->row.templates[layout->row.columns++] = { width, NK_LAYOUT_TOP, NK_LAYOUT_LEFT };
}

NK_API void 
nk_layout_row_template_push_valigned_static(struct nk_context* ctx, float width, float height, nk_layout_valign vAlign)
{
    struct nk_window *win;
    struct nk_panel *layout;
...
nk_layout_row_template_end(struct nk_context *ctx)
{
    ...
    for (i = 0; i < layout->row.columns; ++i) {
        float width = layout->row.templates[i].width;
        if (width >= 0.0f) {
    ...
    struct nk_window *win, int modify)
{
    ...
    case NK_LAYOUT_TEMPLATE: {
        ...
        item_width = layout->row.templates[layout->row.index].width;
        item_offset = layout->row.item_offset;
        item_spacing = (float)layout->row.index * spacing.x;
        if (modify) layout->row.item_offset += item_width;
    } break;
    ...
    struct nk_text_edit *edit, nk_plugin_filter filter)
{
    ...
    nk_emit_hotspot(ctx, bounds, NK_CURSOR_TEXT, 0);
    ...
    if (ctx->last_widget_state & NK_WIDGET_STATE_HOVER)
    {
        ...
        ctx->cursor_style = NK_CURSOR_TEXT;
    }
...
nk_property(struct nk_context *ctx, const char *name, struct nk_property_variant *variant,
    float inc_per_pixel, const enum nk_property_filter filter, nk_prop_color_flags flags, nk_color color, int forceActive)
{
    struct nk_window *win;
    struct nk_panel *layout;
    struct nk_input *in;
    ...
    nk_do_property(ctx, &ctx->last_widget_state, &win->buffer, bounds, name,
        ...
        ctx->button_behavior, flags, color, forceActive);

    if ((in && *state != NK_PROPERTY_DEFAULT && !win->property.active) || forceActive) {
...
nk_property_int(struct nk_context *ctx, const char *name,
    int min, int *val, int max, int step, float inc_per_pixel, nk_prop_color_flags flags, nk_color color, int forceActive)
{
    ...
    int ret = nk_property(ctx, name, &variant, inc_per_pixel, NK_FILTER_INT, flags, color, forceActive);
    *val = variant.value.i;
    return ret;
}
...
nk_property_float(struct nk_context *ctx, const char *name,
    float min, float *val, float max, float step, float inc_per_pixel, nk_prop_color_flags flags, nk_color color, int forceActive)
{
    struct nk_property_variant variant;
    NK_ASSERT(ctx);
    NK_ASSERT(name);
    ...
    int ret = nk_property(ctx, name, &variant, inc_per_pixel, NK_FILTER_FLOAT, flags, color, forceActive);
    *val = variant.value.f;
    return ret;
}
...
nk_property_double(struct nk_context *ctx, const char *name,
    double min, double *val, double max, double step, float inc_per_pixel, nk_prop_color_flags flags, nk_color color, int forceActive)
{
    struct nk_property_variant variant;
    NK_ASSERT(ctx);
    NK_ASSERT(name);
...
nk_propertyi(struct nk_context *ctx, const char *name, int min, int val,
    int max, int step, float inc_per_pixel, nk_prop_color_flags flags, nk_color color)
{
    struct nk_property_variant variant;
    NK_ASSERT(ctx);
    NK_ASSERT(name);
    ...
    variant = nk_property_variant_int(val, min, max, step);
    nk_property(ctx, name, &variant, inc_per_pixel, NK_FILTER_INT, flags, color, 0);
    val = variant.value.i;
    return val;
}
...
nk_propertyf(struct nk_context *ctx, const char *name, float min,
    float val, float max, float step, float inc_per_pixel, nk_prop_color_flags flags, nk_color color)
{
    struct nk_property_variant variant;
    NK_ASSERT(ctx);
    NK_ASSERT(name);
    ...
    if (!ctx || !ctx->current || !name) return val;
    variant = nk_property_variant_float(val, min, max, step);
    nk_property(ctx, name, &variant, inc_per_pixel, NK_FILTER_FLOAT, flags, color, 0);
    val = variant.value.f;
    return val;
}
...
nk_propertyd(struct nk_context *ctx, const char *name, double min,
    double val, double max, double step, float inc_per_pixel, nk_prop_color_flags flags, nk_color color)
{
    struct nk_property_variant variant;
    NK_ASSERT(ctx);
    NK_ASSERT(name);
    ...
    int count, float min_value, float max_value)
{
    ...
    {struct nk_chart_slot *slot = &chart->slots_[chart->slot++];
    slot->type = type;
    slot->count = count;
    slot->color = color;
    ...
    struct nk_chart *g, float value, int slot)
{
    ...
    step = g->w / (float)g->slots_[slot].count;
    range = g->slots_[slot].max - g->slots_[slot].min;
    ratio = (value - g->slots_[slot].min) / range;

    if (g->slots_[slot].index == 0) {
        /* first data point does not have a connection */
        g->slots_[slot].last.x = g->x;
        g->slots_[slot].last.y = (g->y + g->h) - ratio * (float)g->h;

        bounds.x = g->slots_[slot].last.x - 2;
        bounds.y = g->slots_[slot].last.y - 2;
        bounds.w = bounds.h = 4;

        color = g->slots_[slot].color;
        if (!(layout->flags & NK_WINDOW_ROM) &&
            NK_INBOX(i->mouse.pos.x, i->mouse.pos.y, g->slots_[slot].last.x - 3, g->slots_[slot].last.y - 3, 6, 6)) {
            ret = nk_input_is_mouse_hovering_rect(i, bounds) ? NK_CHART_HOVERING : 0;
            ret |= (i->mouse.buttons[NK_BUTTON_LEFT].down &&
                i->mouse.buttons[NK_BUTTON_LEFT].clicked) ? NK_CHART_CLICKED : 0;
            color = g->slots_[slot].highlight;
        }
        nk_fill_rect(out, bounds, 0, color);
        g->slots_[slot].index += 1;
        return ret;
    }

    /* draw a line between the last data point and the new one */
    color = g->slots_[slot].color;
    cur.x = g->x + (float)(step * (float)g->slots_[slot].index);
    cur.y = (g->y + g->h) - (ratio * (float)g->h);
    nk_stroke_line(out, g->slots_[slot].last.x, g->slots_[slot].last.y, cur.x, cur.y, 1.0f, color);
    ...
    g->slots_[slot].last.x = cur.x;
    g->slots_[slot].last.y = cur.y;
    g->slots_[slot].index += 1;
    return ret;
}
    ...
    struct nk_chart *chart, float value, int slot)
{
    ...
    NK_ASSERT(slot >= 0 && slot < NK_CHART_MAX_SLOT);
    if (chart->slots_[slot].index >= chart->slots_[slot].count)
        return nk_false;
    if (chart->slots_[slot].count) {
        float padding = (float)(chart->slots_[slot].count - 1);
        item.w = (chart->w - padding) / (float)(chart->slots_[slot].count);
    }

    /* calculate bounds of current bar chart entry */
    color = chart->slots_[slot].color;;
    item.h = chart->h * NK_ABS((value / chart->slots_[slot].range));
    if (value >= 0) {
        ratio = (value + NK_ABS(chart->slots_[slot].min)) / NK_ABS(chart->slots_[slot].range);
        item.y = (chart->y + chart->h) - chart->h * ratio;
    }
    else {
        ratio = (value - chart->slots_[slot].max) / chart->slots_[slot].range;
        item.y = chart->y + (chart->h * NK_ABS(ratio)) - item.h;
    }
    item.x = chart->x + ((float)chart->slots_[slot].index * item.w);
    item.x = item.x + ((float)chart->slots_[slot].index);
    ...
    if (!(layout->flags & NK_WINDOW_ROM) &&
        ...
        color = chart->slots_[slot].highlight;
    }
    nk_fill_rect(out, item, 0, color);
    chart->slots_[slot].index += 1;
    return ret;
}
...
nk_chart_push_slot(struct nk_context *ctx, float value, int slot)
{
    ...
    switch (win->layout->chart.slots_[slot].type) {
    case NK_CHART_LINES:
