...
ImGuiIO::ImGuiIO()
{
    ...
    // JSandusky, external tooltips
    ShowTooltipUserData = NULL;
    ShowTooltipFn = NULL;
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list)
{
    ...
    if (last_cmd.ElemCount == 0 && last_cmd.UserCallback == NULL && last_cmd.CmdType == IMCMD_Buffer)
    {
...
void ImGui::EndFrame()
{
    ...
    if (g.Tooltip[0])
    {
        // JSandusky: allow external tooltips
        if (g.IO.ShowTooltipFn)
        {
            g.IO.ShowTooltipFn(g.IO.ShowTooltipUserData, g.Tooltip);
        }
        else
        {
            ImGui::BeginTooltip();
    ...
    else if (g.IO.ShowTooltipFn) // if using external tooltips call the function with null to indicate hide status
        g.IO.ShowTooltipFn(g.IO.ShowTooltipUserData, 0x0);
...
void ImGui::RenderCollapseTriangle(ImVec2 p_min, bool is_open, float scale)
{
    ...
    {
        center.y -= r*0.25f;
        a = center + ImVec2(0, 1)*r; //bottom
        b = center + ImVec2(-0.866f, -0.5f)*r; // top-left
        c = center + ImVec2(0.866f, -0.5f)*r;  // top-right
#define FLOOR_VERT(v) v.x = floorf(v.x); v.y = floorf(v.y)
    }
    else
    {
...
bool ImGui::DragFloatN(const char* label, float* v, int components, float v_speed, float v_min, float v_max, const char* display_format, float power, int extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (extra_flags & ImGuiInputTextFlags_Colorize)
        {
            IM_ASSERT(components <= 4);
...
bool ImGui::DragFloat2(const char* label, float v[2], float v_speed, float v_min, float v_max, const char* display_format, float power, int text_flags)
{
    return DragFloatN(label, v, 2, v_speed, v_min, v_max, display_format, power, text_flags);
}

bool ImGui::DragFloat3(const char* label, float v[3], float v_speed, float v_min, float v_max, const char* display_format, float power, int text_flags)
{
    return DragFloatN(label, v, 3, v_speed, v_min, v_max, display_format, power, text_flags);
}

bool ImGui::DragFloat4(const char* label, float v[4], float v_speed, float v_min, float v_max, const char* display_format, float power, int text_flags)
{
    return DragFloatN(label, v, 4, v_speed, v_min, v_max, display_format, power, text_flags);
}
...
bool ImGui::DragIntN(const char* label, int* v, int components, float v_speed, int v_min, int v_max, const char* display_format, int extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::DragInt2(const char* label, int v[2], float v_speed, int v_min, int v_max, const char* display_format, int text_flags)
{
    return DragIntN(label, v, 2, v_speed, v_min, v_max, display_format, text_flags);
}

bool ImGui::DragInt3(const char* label, int v[3], float v_speed, int v_min, int v_max, const char* display_format, int text_flags)
{
    return DragIntN(label, v, 3, v_speed, v_min, v_max, display_format, text_flags);
}

bool ImGui::DragInt4(const char* label, int v[4], float v_speed, int v_min, int v_max, const char* display_format, int text_flags)
{
    return DragIntN(label, v, 4, v_speed, v_min, v_max, display_format, text_flags);
}
