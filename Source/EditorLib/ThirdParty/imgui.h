...
namespace ImGui
{
    ...
    IMGUI_API bool          DragFloat2(const char* label, float v[2], float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* display_format = "%.3f", float power = 1.0f, int text_flags = 0);
    IMGUI_API bool          DragFloat3(const char* label, float v[3], float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* display_format = "%.3f", float power = 1.0f, int text_flags = 0);
    IMGUI_API bool          DragFloat4(const char* label, float v[4], float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* display_format = "%.3f", float power = 1.0f, int text_flags = 0);
    IMGUI_API bool          DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed = 1.0f, float v_min = 0.0f, float v_max = 0.0f, const char* display_format = "%.3f", const char* display_format_max = NULL, float power = 1.0f);
    IMGUI_API bool          DragInt(const char* label, int* v, float v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* display_format = "%.0f");                                       // If v_min >= v_max we have no bound
    IMGUI_API bool          DragInt2(const char* label, int v[2], float v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* display_format = "%.0f", int text_flags = 0);
    IMGUI_API bool          DragInt3(const char* label, int v[3], float v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* display_format = "%.0f", int text_flags = 0);
    IMGUI_API bool          DragInt4(const char* label, int v[4], float v_speed = 1.0f, int v_min = 0, int v_max = 0, const char* display_format = "%.0f", int text_flags = 0);
...
enum ImGuiInputTextFlags_
{
    ...
    // JSandusky
    ImGuiInputTextFlags_Colorize = 1 << 16, // will draw R, G, B underlines for input editors (only applies to Input___2, Input___3, and Input___4
                                             // [Internal]
...
struct ImGuiIO
{
    ...
    // JSandusky, Optional: use external tooltip output
    void* ShowTooltipUserData;
    void(*ShowTooltipFn)(void* user_data, const char* text);
...
public:
    ...
    struct RectInfo {
        ...
        ImU32           LeftColor;
        ImU32           RightColor;
        float           Rounding; // if non-zero than it is square
        bool            Stroke;
    };
    struct TextInfo {
        const char* Text;
    ...
    ImDrawCmd() {
        ...
        TextureId = NULL; UserCallback = NULL; UserCallbackData = NULL; 
        Font = 0x0; 
        CmdType = IMCMD_Buffer;
    }

    ~ImDrawCmd() {
        if (CmdType == IMCMD_Text)
            delete[] CmdData.TextData.Text;
    }
};
...
struct ImDrawList
{
    ...
    // Advanced
    IMGUI_API void  CheckCmdSafety();                                           // Checks if we need to add a draw cmd because the previous one is text/etc
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              AddExternalFont(ImFont* font);
...
struct ImFont
{
    ...
    virtual IMGUI_API const Glyph*      FindGlyph(ImWchar c) const;
    ...
    virtual IMGUI_API ImVec2    CalcTextSizeA(float size, float max_width, float wrap_width, const char* text_begin, const char* text_end = NULL, const char** remaining = NULL) const; // utf8
    virtual IMGUI_API const char* CalcWordWrapPositionA(float scale, const char* text, const char* text_end, float wrap_width) const;
    virtual IMGUI_API void      RenderChar(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, unsigned short c) const;
    virtual IMGUI_API void      RenderText(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false) const;
