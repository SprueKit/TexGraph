...
#endif
#ifndef IMGUI_DISABLE_STB_RECT_PACK
...
//-----------------------------------------------------------------------------
// ImDrawList additions by JSandusky
//-----------------------------------------------------------------------------

void PushText(ImDrawList* drawList, ImVec2 pos, ImU32 color, const char* start, int len, ImVec4 clipRect)
{
    drawList->AddDrawCmd();
    ImDrawCmd& cmd = drawList->CmdBuffer.back();
    cmd.CmdType = IMCMD_Text;
    char* buffer = new char[len + 1];
    memset(buffer, 0, len + 1);
    strncpy(buffer, start, len);
    cmd.ClipRect = clipRect;
    cmd.CmdData.TextData.Text = buffer;
    cmd.CmdData.TextData.Length = len;
    cmd.CmdData.TextData.Position = pos;
    cmd.CmdData.TextData.Color = color;
}

void PushRect(ImDrawList* drawList, ImVec2 a, ImVec2 b, ImU32 color, float rounding, float thickness)
{
    drawList->AddDrawCmd();
    ImDrawCmd& cmd = drawList->CmdBuffer.back();
    cmd.CmdType = IMCMD_Rect;
    //drawList->CmdBuffer.back().ClipRect = clipRect;
    cmd.CmdData.RectData.LeftColor = color;
    cmd.CmdData.RectData.RightColor = color;
    cmd.CmdData.RectData.Rectangle.x = a.x;
    cmd.CmdData.RectData.Rectangle.y = a.y;
    cmd.CmdData.RectData.Rectangle.z = b.x;
    cmd.CmdData.RectData.Rectangle.w = b.y;
    cmd.CmdData.RectData.Rounding = rounding;
    cmd.CmdData.RectData.Stroke = thickness;
}

void PushGradientRect(ImDrawList* drawList, ImVec2 a, ImVec2 b, ImU32 leftCol, ImU32 rightCol, float rounding, float thickness)
{
    drawList->AddDrawCmd();
    ImDrawCmd& cmd = drawList->CmdBuffer.back();
    cmd.CmdType = IMCMD_Rect;
    //drawList->CmdBuffer.back().ClipRect = clipRect;
    cmd.CmdData.RectData.LeftColor = leftCol;
    cmd.CmdData.RectData.RightColor = rightCol;
    cmd.CmdData.RectData.Rectangle.x = a.x;
    cmd.CmdData.RectData.Rectangle.y = a.y;
    cmd.CmdData.RectData.Rectangle.z = b.x;
    cmd.CmdData.RectData.Rectangle.w = b.y;
...
void PushCircle(ImDrawList* drawList, ImVec2 center, float radius, ImU32 color, float thickness)
{
    drawList->AddDrawCmd();
    ImDrawCmd& cmd = drawList->CmdBuffer.back();
    cmd.CmdType = IMCMD_Circle;
    //drawList->CmdBuffer.back().ClipRect = clipRect;
    cmd.CmdData.CircleData.Color = color;
    cmd.CmdData.CircleData.Center = center;
    cmd.CmdData.CircleData.Radius = radius;
    cmd.CmdData.CircleData.Color = color;
    cmd.CmdData.CircleData.Stroke = thickness;
}

void PushLine(ImDrawList* drawList, ImVec2 a, ImVec2 b, ImU32 color, float thickness)
{
    drawList->AddDrawCmd();
    ImDrawCmd& cmd = drawList->CmdBuffer.back();
    cmd.CmdType = IMCMD_Line;
    //drawList->CmdBuffer.back().ClipRect = clipRect;
    cmd.CmdData.LineData.Color = color;
    cmd.CmdData.LineData.A = a;
    cmd.CmdData.LineData.B = b;
    cmd.CmdData.LineData.Color = color;
    cmd.CmdData.LineData.Width = thickness;
}

void PushImage(ImDrawList* drawList, ImVec4 rect, ImTextureID id)
{
    drawList->AddDrawCmd();
    drawList->CmdBuffer.back().CmdType = IMCMD_Image;
    drawList->CmdBuffer.back().CmdData.ImageData.rect_ = rect;
    drawList->CmdBuffer.back().CmdData.ImageData.textureID_ = id;
}
...
void ImDrawList::Clear()
{
    ...
    {
        if (CmdBuffer[i].CmdType == IMCMD_Text)
            delete[] CmdBuffer[i].CmdData.TextData.Text;
    }
    CmdBuffer.resize(0);
    IdxBuffer.resize(0);
    VtxBuffer.resize(0);
...
void ImDrawList::CheckCmdSafety()
{
    if (CmdBuffer.size() && CmdBuffer.back().CmdType != IMCMD_Buffer)
        AddDrawCmd();
}
...
void ImDrawList::UpdateClipRect()
{
    ...
    if (!curr_cmd || (curr_cmd->ElemCount != 0 && memcmp(&curr_cmd->ClipRect, &curr_clip_rect, sizeof(ImVec4)) != 0) || curr_cmd->UserCallback != NULL || curr_cmd->CmdType != IMCMD_Buffer)
    {
        AddDrawCmd();
        return;
    }
...
void ImDrawList::AddLine(const ImVec2& a, const ImVec2& b, ImU32 col, float thickness)
{
    if ((col & IM_COL32_A_MASK) == 0)
        return;
    PushLine(this, a, b, col, thickness);
    //CheckCmdSafety();
...
void ImDrawList::AddRect(const ImVec2& a, const ImVec2& b, ImU32 col, float rounding, int rounding_corners_flags, float thickness)
{
    if ((col & IM_COL32_A_MASK) == 0)
        return;
    PushRect(this, a, b, col, rounding, thickness);
    //CheckCmdSafety();
...
void ImDrawList::AddRectFilled(const ImVec2& a, const ImVec2& b, ImU32 col, float rounding, int rounding_corners_flags)
{
    ...
    if (rounding > 0.0f)
    {
        PushRect(this, a, b, col, rounding, 0);
        //PathRect(a, b, rounding, rounding_corners_flags);
        //PathFillConvex(col);
    }
    else
    {
        PushRect(this, a, b, col, 0, 0);
        //PrimReserve(6, 4);
        //PrimRect(a, b, col);
    }
}
...
void ImDrawList::AddRectFilledMultiColor(const ImVec2& a, const ImVec2& c, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left)
{
    ...
    if (col_upr_left == col_bot_left && col_upr_right == col_bot_right)
    {
        PushGradientRect(this, a, c, col_upr_left, col_upr_right, 0, 0);
        return;
    }
...
void ImDrawList::AddCircle(const ImVec2& centre, float radius, ImU32 col, int num_segments, float thickness)
{
    if ((col & IM_COL32_A_MASK) == 0)
        return;
    PushCircle(this, centre, radius, col, thickness);
    //CheckCmdSafety();
...
void ImDrawList::AddCircleFilled(const ImVec2& centre, float radius, ImU32 col, int num_segments)
{
    if ((col & IM_COL32_A_MASK) == 0)
        return;
    PushCircle(this, centre, radius, col, 0);
    //CheckCmdSafety();
    //const float a_max = IM_PI*2.0f * ((float)num_segments - 1.0f) / (float)num_segments;
...
void ImDrawList::AddImage(ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b, const ImVec2& uv_a, const ImVec2& uv_b, ImU32 col)
{
    ...
    PushImage(this, { a.x, a.y, b.x, b.y }, user_texture_id);
#if 0
...
void ImDrawList::AddImageQuad(ImTextureID user_texture_id, const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d, const ImVec2& uv_a, const ImVec2& uv_b, const ImVec2& uv_c, const ImVec2& uv_d, ImU32 col)
{
    ...
    IM_ASSERT(!"ImDrawList::AddImageQuad is not supported");
#if 0
    const bool push_texture_id = _TextureIdStack.empty() || user_texture_id != _TextureIdStack.back();
    if (push_texture_id)
        PushTextureID(user_texture_id);
...
void ImFontAtlas::AddExternalFont(ImFont* font)
{
    Fonts.push_back(font);
...
ImFont* ImFontAtlas::AddFontDefault(const ImFontConfig* font_cfg_template)
{
    ...
    return font;
#else
    IM_ASSERT(!"ImFontAtlas::AddFontDefault, not supported");
    return 0x0;
#endif
}
...
#else
    IM_ASSERT(!"ImFontAtlas::AddFontFromMemoryCompressedTTF, not supported");
    return 0x0;
#endif
}
...
void ImFont::RenderChar(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, unsigned short c) const
{
    if (c == ' ' || c == '\t' || c == '\n' || c == '\r') // Match behavior of RenderText(), those 4 codepoints are hard-coded.
        return;
    IM_ASSERT(!"ImFont::RenderChar is not supported");
    ...
    while (s < text_end)
    {
        if (word_wrap_enabled)
        {
            ...
            if (s >= word_wrap_eol)
            {
                ...
                if (s != currentScan)
                {
                    PushText(draw_list, { pos.x, y }, col, currentScan, s - currentScan, clip_rect);
                    currentScan = s;
                }
        ...
        if (const Glyph* glyph = FindGlyph((unsigned short)c))
        {
            ...
            if (c != ' ' && c != '\t')
            {
                //IM_ASSERT(!"JSandusky: RenderText not implemented yet");
