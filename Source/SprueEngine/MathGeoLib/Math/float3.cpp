...
#include "float3_packing_table.inl"

using namespace std;

const float3 float3::PositiveX(1,0,0);
const float3 float3::PositiveY(0,1,0);
const float3 float3::PositiveZ(0,0,1);
const float3 float3::NegativeX(-1,0,0);
const float3 float3::NegativeY(0,-1,0);
const float3 float3::NegativeZ(0,0,-1);
...
float3 float3::ProjectOntoPlane(const float3& planeNormal) const {
    float3 n = Normalized();
    float3 p = planeNormal.Normalized();
    return (n - (p * n.Dot(p))).Normalized();
}

float3 float3::AngleLimitedDeg(const float3& baseline, float angleLimit) const
{
    float angleBetween = AngleDeg(baseline);
    if (angleBetween > angleLimit)
    {
        float3 corrected = baseline.Normalized().Cross(Normalized()).Normalized();
        return RotateAroundAxis(angleLimit, corrected);
    }
    return Normalized();
}

float3 float3::RotateAroundAxis(float degrees, const float3& axis) const
{
    return Quat(axis, degrees * 0.0174533f) * *this;
}
...
/// From http://www.gamedev.net/page/resources/_/technical/math-and-physics/higher-accuracy-quantized-normals-r1252
/// Original author: Rafael Baptista
uint16_t float3::ToPackedNormal() const
{
    float3 tmp = *this;

    uint16_t packed = 0;

    if (tmp.x < 0) { packed |= XSIGN_MASK; tmp.x = -tmp.x; }
    if (tmp.y < 0) { packed |= YSIGN_MASK; tmp.y = -tmp.y; }
    if (tmp.z < 0) { packed |= ZSIGN_MASK; tmp.z = -tmp.z; }

    float w = 126.0f / (tmp.x + tmp.y + tmp.z);
    long xbits = (long)(tmp.x * w);
    long ybits = (long)(tmp.y * w);

    if (xbits >= 64) { xbits = 127 - xbits; ybits = 127 - ybits; }
...
void float3::FromPackedNormal(uint16_t packed)
{
    long xbits = ((packed & TOP_MASK) >> 7);
    long ybits = (packed & BOTTOM_MASK);

    if ((xbits + ybits) >= 127) { xbits = 127 - xbits; ybits = 127 - ybits; }

    float uvadj = PackedFloat3UVAdjustment[packed & ~SIGN_MASK];
    x = uvadj * (float)xbits;
    y = uvadj * (float)ybits;
    z = uvadj * (float)(126 - xbits - ybits);

    // set all the sign bits
    if (packed & XSIGN_MASK) x = -x;
    if (packed & YSIGN_MASK) y = -y;
    if (packed & ZSIGN_MASK) z = -z;
}
