...
public:
    ...
    float3& operator+=(float scalar) { Set(x + scalar, y + scalar, z + scalar); return *this; }
    float3& operator-=(float scalar) { Set(x - scalar, y - scalar, z - scalar); return *this; }
    ...
    // JSandusky modifications for SprueKit
    static float3 MaxVector(float3 lhs, float3 rhs) { return float3(std::max(lhs.x, rhs.x), std::max(lhs.y, rhs.y), std::max(lhs.z, rhs.z)); }
    static float3 MinVector(float3 lhs, float3 rhs) { return float3(std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y), std::min(lhs.z, rhs.z)); }
...
#define CLOSE_ENOUGH(lhs, rhs) ((lhs) + FLT_EPSILON >= (rhs) && (lhs) - FLT_EPSILON <= (rhs))
    inline bool operator<(const float3& rhs) const {
        return std::tuple<float,float,float>(x, y, z) < std::tuple<float,float,float>(rhs.x, rhs.y, rhs.z);
        //if (!CLOSE_ENOUGH(x, rhs.x))
        //    return x < rhs.x;
        //if (!CLOSE_ENOUGH(y, rhs.y))
        //    return y < rhs.y;
        //return z < rhs.z;
    }
    inline bool operator>(const float3& rhs) const { 
...
#undef CLOSE_ENOUGH
    ...
    static const float3 PositiveX;
    static const float3 PositiveY;
    static const float3 PositiveZ;
    static const float3 NegativeX;
    static const float3 NegativeY;
    static const float3 NegativeZ;

    float3 ProjectOntoPlane(const float3& planeNormal) const;
    inline float AngleDeg(const float3& rhs) const { return acosf(Dot(rhs)) * 57.2958f; }
    float SignedAngleDeg(const float3& rhs, const float3& norm) const {
        float unsignedAngle = AngleDeg(rhs);
        float sign = Cross(rhs).Dot(norm) >= 0.0f ? 1.0f : -1.0f;
        return unsignedAngle * sign;
    }
    float3 AngleLimitedDeg(const float3& baseline, float angleLimit) const;
    float3 RotateAroundAxis(float degrees, const float3& axis) const;

    uint16_t ToPackedNormal() const;
    void FromPackedNormal(uint16_t);
};
