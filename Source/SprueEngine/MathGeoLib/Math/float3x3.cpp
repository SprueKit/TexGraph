...
void float3x3::SetRotation(float degrees)
{
    float cos = (float)cosf(degrees * 0.0174533f);
    float sin = (float)sinf(degrees * 0.0174533f);

    v[0][0] = cos;  v[0][1] = -sin;  v[0][2] = 0;
    v[1][0] = sin;  v[1][1] = cos;   v[1][2] = 0;
    v[2][0] = 0;    v[2][1] = 0;     v[2][2] = 1;
}

float3x3 float3x3::Scaled(const float3& scale)
{
    return float3x3(
        v[0][0] * scale.x,
        v[0][1] * scale.y,
