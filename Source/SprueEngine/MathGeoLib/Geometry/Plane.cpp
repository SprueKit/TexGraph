...
int Plane::Clip(const Triangle& triangle, Triangle &t1, Triangle &t2, Triangle& t3) const
{
    bool side[3];
    side[0] = IsOnPositiveSide(triangle.a);
    side[1] = IsOnPositiveSide(triangle.b);
    ...
    float t_CA = 0.0f;
    bool split[3];
    split[0] = Intersects(triangle.Edge(0), &t_AB);
    split[1] = Intersects(triangle.Edge(1), &t_BC);
    split[2] = Intersects(triangle.Edge(2), &t_CA);
    vec points[3];
    points[0] = triangle.Edge(0).GetPoint(t_AB);
    points[1] = triangle.Edge(1).GetPoint(t_BC);
    points[2] = triangle.Edge(2).GetPoint(t_CA);

    t1 = t2 = t3 = triangle;

    LineSegment line;
    // AB and BC = B is new A
    if (split[0] && split[1])
    {
        t1.a = triangle.b;
        t1.b = points[1];
