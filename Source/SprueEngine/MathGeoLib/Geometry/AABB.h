...
public:
    ...
    vec Normalized(const vec& in) const { return (in-minPoint) / (maxPoint - minPoint); }
...
//	Polyhedron Intersection(const Polyhedron &polyhedron) const;
    ...
    void Subdivide(AABB* list) const;
    void Subdivide(AABB* list, unsigned xDiv, unsigned yDiv, unsigned zDiv) const;
    AABB Shrink(const float3& by) const;
};
