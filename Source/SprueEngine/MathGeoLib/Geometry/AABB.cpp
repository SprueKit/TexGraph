...
void AABB::Subdivide(AABB* list) const
{
    const float3 center = Centroid();
...
void AABB::Subdivide(AABB* list, unsigned xDiv, unsigned yDiv, unsigned zDiv) const
{
    float3 currentMin = minPoint;
    const float3 cellSize = Size() / float3(xDiv, yDiv, zDiv);

    for (unsigned x = 0; x < xDiv; ++x)
    {
        for (unsigned y = 0; y < yDiv; ++y)
        {
            for (unsigned z = 0; z < zDiv; ++z)
            {
                const unsigned idx = SprueEngine::ToArrayIndex(x, y, 0, xDiv, yDiv, zDiv);
                list[idx].minPoint = minPoint + float3(cellSize.x * x, cellSize.y * z, cellSize.z * z);
                list[idx].maxPoint = minPoint + cellSize;
            }
        }
    }
}

AABB AABB::Shrink(const float3& by) const
{
    AABB ret(*this);
    ret.minPoint += by / 2;
    ret.maxPoint -= by / 2;
