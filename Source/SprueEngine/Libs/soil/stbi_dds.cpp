...
int      stbi_dds_test_memory(const unsigned char* buffer, int len)
{
    stbi s;
...
static unsigned char *dds_load(stbi *s, int *x, int *y, int *comp, int req_comp)
{
...
#ifndef STBI_NO_STDIO
unsigned char *stbi_dds_load_from_file(FILE *f, int *x, int *y, int *comp, int req_comp)
{
    stbi s;
    start_file(&s, f);
...
unsigned char *stbi_dds_load(char *filename, int *x, int *y, int *comp, int req_comp)
{
    unsigned char *data;
...
unsigned char *stbi_dds_load_from_memory(unsigned char const *buffer, int len, int *x, int *y, int *comp, int req_comp)
{
    stbi s;
    start_mem(&s, buffer, len);
    return dds_load(&s, x, y, comp, req_comp);
