    ...
    public:
        ...
        JacobiPreconditioner(const SparseMatrix & M, bool symmetric) : m_inverseDiagonal(M.width())
        {
            ...
            for(uint32_t x = 0; x < M.width(); x++)
            {
                ...
                ////nvCheck( elem != 0.0f ); // This can be zero in the presence of zero area triangles.
        ...
        {
            //nvCheck(x.dimension() == m_inverseDiagonal.dimension());
            //nvCheck(y.dimension() == m_inverseDiagonal.dimension());
...
{
    //nvCheck(A.width() == x.dimension());
    //nvCheck(A.height() == b.dimension());
    //nvCheck(A.height() >= A.width()); // @@ If height == width we could solve it directly...
...
bool nv::LeastSquaresSolver(const SparseMatrix & A, const FullVector & b, FullVector & x, const uint32_t * lockedParameters, uint32_t lockedCount, float epsilon/*= 1e-5f*/)
{
    //nvCheck(A.width() == x.dimension());
    //nvCheck(A.height() == b.dimension());
    //nvCheck(A.height() >= A.width() - lockedCount);
...
{
    //nvCheck(A.height() == A.width());
    //nvCheck(A.height() == b.dimension());
    //nvCheck(b.dimension() == x.dimension());
