...
namespace nv
{
    ...
    inline uint32_t countSetBits(uint32_t x) {
        uint32_t count = 0;
        for(; x != 0; x >>= 1) {
            count += (x & 1);
        }
    ...
    inline uint32_t countSetBits(uint32_t x, int bits) {
        uint32_t count = 0;
    ...
    public:
        ...
        void resize(uint32_t new_size, bool init)
        {
            //if (new_size == m_size) return;

            uint32_t old_size = m_size;
            uint32_t size_mod_32 = old_size & 31;
            uint32_t last_word_index = ((old_size + 31) >> 5) - 1;
            uint32_t mask = (1 << size_mod_32) - 1;
        ...
        void toggleAll()
        {
            const uint32_t wordCount = m_wordArray.size();
            for (uint32_t b = 0; b < wordCount; b++) {
        ...
        {
            const uint32_t num = m_wordArray.size();
            if( num == 0 ) {
                return 0;
            }
        ...
        friend void swap(BitArray & a, BitArray & b)
        {
            ...
            std::swap(a.m_wordArray, b.m_wordArray);
        }
