...
ParameterizationQuality::ParameterizationQuality(const HalfEdge::Mesh * mesh)
{
    ...
    for (uint32_t f = 0; f < faceCount; f++)
    {
        ...
        SprueEngine::Vec3 p[3];
        SprueEngine::Vec2 t[3];
...
void ParameterizationQuality::processTriangle(SprueEngine::Vec3 q[3], SprueEngine::Vec2 p[3])
{
    ...
    float geometricArea = (q[1] - q[0]).Cross(q[2] - q[0]).Length() / 2;
    float parametricArea = ((s2 - s1)*(t3 - t1) - (s3 - s1)*(t2 - t1)) / 2;
    
    if (SprueEquals(parametricArea, 0))
    {
        m_zeroAreaTriangleCount++;
        return;
    }

    SprueEngine::Vec3 Ss = (q[0] * (t2- t3) + q[1] * (t3 - t1) + q[2] * (t1 - t2)) / (2 * parametricArea);
    SprueEngine::Vec3 St = (q[0] * (s3- s2) + q[1] * (s1 - s3) + q[2] * (s2 - s1)) / (2 * parametricArea);
...
#define square(VALUE) ((VALUE) * (VALUE))
    ...
    if (!SprueEquals(sigma1, 0.000001f)) {
