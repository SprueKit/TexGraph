...
namespace nv
{
    ...
    public:
        ...
        uint32_t meshCount() const { return m_meshChartsArray.size(); }
        const MeshCharts * meshAt(uint32_t i) const { return m_meshChartsArray[i]; }
        MeshCharts * meshAt(uint32_t i) { return m_meshChartsArray[i]; }
    ...
    public:
        ...
        uint32_t vertexCount() const { return m_totalVertexCount; }

        const Chart * chartAt(uint32_t i) const { return m_chartArray[i]; }
        Chart * chartAt(uint32_t i) { return m_chartArray[i]; }
        ...
        uint32_t faceChartAt(uint32_t i) const { return m_faceChart[i]; }
        uint32_t faceIndexWithinChartAt(uint32_t i) const { return m_faceIndex[i]; }

        uint32_t vertexCountBeforeChartAt(uint32_t i) const { return m_chartVertexCountPrefixSum[i]; }
    ...
    public:
        ...
        uint32_t faceAt(uint32_t i) const { return m_faceArray[i]; }
        ...
        uint32_t mapChartVertexToUnifiedVertex(uint32_t i) const { return m_chartToUnifiedMap[i]; }

        const std::vector<uint32_t> & faceArray() const { return m_faceArray; }

        void transferParameterization();

        float computeSurfaceArea() const;
        float computeParametricArea() const;
        SprueEngine::Vec2 computeParametricBounds() const;
