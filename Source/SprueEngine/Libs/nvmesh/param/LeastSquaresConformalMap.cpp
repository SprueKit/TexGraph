...
namespace
{
    ...
    static void findDiameterVertices(HalfEdge::Mesh * mesh, HalfEdge::Vertex ** a, HalfEdge::Vertex ** b)
    {
        ...
        for (uint32_t v0 = 1; v0 < vertexCount; v0++)
        {
            ...
            for (uint32_t v1 = 0; v1 < v0; v1++)
            {
                ...
                float len = (vertex0->pos - vertex1->pos).Length();
    ...
    static bool findApproximateDiameterVertices(HalfEdge::Mesh * mesh, HalfEdge::Vertex ** a, HalfEdge::Vertex ** b)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            lengths[i] = (minVertex[i]->pos - maxVertex[i]->pos).Length();
        }
    ...
    static void project_triangle(SprueEngine::Vec3 p0, SprueEngine::Vec3 p1, SprueEngine::Vec3 p2, SprueEngine::Vec2 * z0, SprueEngine::Vec2 * z1, SprueEngine::Vec2 * z2)
    {
        SprueEngine::Vec3 X = (p1 - p0).Normalized();
        SprueEngine::Vec3 Z = X.Cross(p2 - p0).Normalized();
        SprueEngine::Vec3 Y = Z.Cross(X).Normalized();

        float x0 = 0.0f;
        float y0 = 0.0f;
        float x1 = (p1 - p0).Length();
        float y1 = 0.0f;
        float x2 = (p2 - p0).Dot(X);
        float y2 = (p2 - p0).Dot(Y);

        *z0 = SprueEngine::Vec2(x0, y0);
        *z1 = SprueEngine::Vec2(x1, y1);
        *z2 = SprueEngine::Vec2(x2, y2);
    }
    ...
    static void setup_conformal_map_relations(SparseMatrix & A, int row, const HalfEdge::Vertex * v0, const HalfEdge::Vertex * v1, const HalfEdge::Vertex * v2)
    {
        ...
        SprueEngine::Vec3 p0 = v0->pos;
        SprueEngine::Vec3 p1 = v1->pos;
        SprueEngine::Vec3 p2 = v2->pos;

        SprueEngine::Vec2 z0, z1, z2;
        project_triangle(p0, p1, p2, &z0, &z1, &z2);

        SprueEngine::Vec2 z01 = z1 - z0;
        SprueEngine::Vec2 z02 = z2 - z0;
    ...
    static float vec_angle_cos(SprueEngine::Vec3 v1, SprueEngine::Vec3 v2, SprueEngine::Vec3 v3)
    {
        SprueEngine::Vec3 d1 = v1 - v2;
        SprueEngine::Vec3 d2 = v3 - v2;
        return CLAMP(d1.Dot(d2) / (d1.Length() * d2.Length()), -1.0f, 1.0f);
    }

    static float vec_angle(SprueEngine::Vec3 v1, SprueEngine::Vec3 v2, SprueEngine::Vec3 v3)
    {
        float dot = vec_angle_cos(v1, v2, v3);
        return acosf(dot);
    }

    static void triangle_angles(SprueEngine::Vec3 v1, SprueEngine::Vec3 v2, SprueEngine::Vec3 v3, float *a1, float *a2, float *a3)
    {
    ...
    static void triangle_cosines(SprueEngine::Vec3 v1, SprueEngine::Vec3 v2, SprueEngine::Vec3 v3, float *a1, float *a2, float *a3)
    {
...
bool nv::computeLeastSquaresConformalMap(HalfEdge::Mesh * mesh)
{
    ...
    for (uint32_t v = 0; v < vertexCount; v++)
    {
        ...
        vertex->tex = SprueEngine::Vec2(x[2 * v + 0], x[2 * v + 1]);
    }
